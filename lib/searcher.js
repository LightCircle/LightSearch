...
exports.add = function(code, collection, id, docs, callback) {
  ...
  var updateSource = function(data, idf, done) {
    ...
    db.update({ _id: id }, { $set: {idx: hash} }, { upsert: true },
      function(err, result) {
        done(err, result);
      });
  };
...
exports.similarSearch = function(code, collection, text, condition, callback) {
  ...
  var find = function(words, idf, done) {
    ...
    _.each(words, function(word) {
      var condition = {};
      condition["idx." + word + ".k"] = {$exists: true};
      or.push(condition);
      sort["idx." + word + ".v.tfidf"] = -1; // TODO: 应该用tfidf的合
    });
    ...
    console.log(filter);
    db.find(filter, { sort: sort, limit: 50}
      , function(err, result) {
        result.toArray(function(err, result) {
...
exports.fullTextSearch = function(code, collection, text, condition, callback) {
  ...
  var find = function(words, done) {
    ...
    _.each(words, function(word) {
      filter["idx." + word] = {$exists: true};
      sort["idx." + word + ".v.tf"] = -1;
    });

    db.find(filter, {fields: ["_id"]}, {sort: sort, skip: 0, limit: 50}, function(err, result) {
      result.toArray(function(err, result) {
