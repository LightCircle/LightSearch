...
var light       = require("light-framework")
  ...
  , splitter    = require("./mecab")//require("./ictclas")
  ;
...
// 相似检索
exports.similarSearch = function(code, collection, text, callback) {
  ...
  var find = function(words, idf, done) {
    ...
    db.find({$or: or}, {sort: {"idx.v.tf": 1}, limit: 50}
      , function(err, result) {
        result.toArray(function(err, result) {
          done(err, idf, result);
        });
  ...
  // 计算相似度(需要遍历所有得结果集)
  var similar = function(idf, dbdata, done) {
    done(undefined, cosine(idf, dbdata));
  };

  async.waterfall([split, calcIdf, find, similar], function (err, result) {
    callback(err, result);
  });
 ...
 * @param callback
 */
exports.fullTextSearch = function(code, collection, text, callback) {
  ...
  var find = function(words, done) {
    db.find({"idx.k": {$all: words}}, {fields: {_id: 1}}, {sort: ["idx.v.tf", 1]}, function(err, result) {
      result.toArray(function(err, result) {
        done(err, result);
      });
    });
  ...
  async.waterfall([split, find], function (err, result) {
    callback(err, result);
  });
};
