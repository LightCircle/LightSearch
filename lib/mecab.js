...
exports.parse = function(docs, callback) {
  ...
  _.each(docs, function(info) {
    ...
    _.each(mecab.parseSync(text), function (term) {
      ...
      // reading       : 8 片假名读音
      // pronunciation : 9 发音
      var kanji = parseRestrictions(term[0])
        , lexical = term[1], compound = term[2], original = term[7], reading = term[8];

      // 根据词性过滤单词
      if (pos[lexical] && pos[lexical][compound] && kanji) {
 ...
 * @returns {undefined}
 */
function parseRestrictions(keyword) {
  ...
  var word = _.str.trim(keyword).replace(/(^\$|\.)/g, "");
  return _.str.isBlank(word) ? undefined : word;
}
